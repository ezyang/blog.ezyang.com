---
title: "What is Stateless User Interface?"
date: 2015-11-27 01:53:49
slug: what-is-stateless-user-interface
categories: [Haskell]
comments:
    - id: 19368
      author: gasche
      date: "2015-11-27 14:44:08"
      content: |
        The git model has an monotone database of immutable objects, plus a set of mutable references (heads) that are used for convenience. We *could* do everything only on the object store (maybe people would like to use more convenient names than the sha1 hashes then, so some design would be necessary), and the ability to inspect the immutable store and act on it is definitely present and useful. But in practice the common use-case of "do something creating a new state and make all future commands act on it by default" is encapsulated by a convenient stateful interface.
        
        This concept of immutable store plus mutable names is surprisingly common -- most content-based addressing systems support some form of it.
    - id: 19369
      author: Edward Z. Yang
      date: "2015-11-27 15:47:09"
      content: "Yes indeed! And it's worth saying that the reasons why the mutable references are \"acceptable\" is because it is a very small amount of state per reference: just the hash that is identified. (And even then, Git is still confusing for new users who haven't internalized this model of Git's state.)"
    - id: 19431
      author: Optional
      date: "2015-11-30 18:23:06"
      content: "An example for a stateless user interface is a picture. And yes, it's many times easier to understand than for example a film."
    - id: 19433
      author: Tom Ellis
      date: "2015-12-01 02:01:13"
      content: "A critical point, that still seems hugely underappreciated in the GHC-using community, is that every package can live in its own database, and thus we can easily reproduce the equivalent of persistant data structures in the GHC-installed-package world.  Nothing ever needs to be mutated or removed, and we can have the equivalent of garbage collection to remove old packages."
---
...
Sometimes the state is merely being used as a *cache*. If a cache is blown away, everything should still work, just more slowly. The package database (reason 1) and configuration cache (reason 4) both fall under this banner, but the critical mistake today's Cabal makes is that if you delete this information, things do *not* "just work". There must be sufficient information to rebuild the cache; e.g., the configuration cache should be supplemented with the actual input to the configure step. (Sometimes, separation of concerns means you simply cannot achieve this. What is `ghc` to do if you ask it to use the not-in-cache <span class="title-ref">lens</span> package?) Furthermore, the behavior of a system should not vary depending on whether or not the cached data is present or not; e.g., the solver (reason 2) should not make different (semantically meaningful) decisions based on what is cached or not.
