---
title: "Use The Monoid: A worked example"
date: 2010-05-28 09:00:14
slug: use-the-monoid
categories: [Haskell]
comments:
    - id: 453
      author: RayNbow
      date: "2010-05-28 14:59:27"
      content: "Small typo in your Python code. Last line should read \"return ret\"."
    - id: 454
      author: Edward Z. Yang
      date: "2010-05-28 16:06:35"
      content: "Fixed, thanks!"
    - id: 455
      author: solrize
      date: "2010-05-28 17:32:32"
      content: "I think I'd have golfed the Python by 1) making \"exports\" always a list (empty in the case where there are no exports), and 2) making \"ret\" be defaultdict(set) to get rid of \"add\"."
    - id: 456
      author: Edward Z. Yang
      date: "2010-05-28 17:59:19"
      content: "defaultdict! Why was I not informed? Updated accordingly. As for making exports always a list, there is a distinction between lacking an exports line altogether, versus having an exports line, but with zero elements. So it's a meaningful distinction (although I guess I could see someone introducing a boolean variable to track it.)"
    - id: 477
      author: "Chris Lesniewski-Laas"
      date: "2010-06-01 15:14:05"
      content: "Alternatively, you could generalize Map to MonoidMap, with the same semantics as the built-in instance Monoid b =&gt; Monoid (a-&gt;b)."
    - id: 22110
      author: SamEtMax
      date: "2017-09-14 08:25:02"
      content: |
        Hey,
        
        Trying to come around this 7 years old article to wrap my head around monads, so thanks your for that article.
        
        I'll take the opportunity to provide a stateless Python version for people needing a way to compare apples to apples:
        
        <pre>
        from itertools import groupby
        
        
        def handle_exports(exports)
            for export in module.exports:
                if any(map(instance, (EVar, EAbs, EThingAll)):
                    yield export.name, module.name
                elif isinstance(export, EThingWith):
                    yield export.name, module.name
                    for cname in export.cnames:
                        yield export.name, cname.name
        
        
        def get_public_names(module, ret=None):
            if module.exports is None:
                return ret
        
            exports = sorted(handle_exports(module.exports))
            exports = groupby(exports, lambda names: names[0])
            return {mod: {n for m, n in names} for mod, names in exports}
        </pre>
        
        It's lazy up to sorted(), which is side effect free, but does create a whole list in memory. All in all it will probably be less performant than the stateful version and it will certainly be always harder to read. But it's good to know what's your options are.
---
