...
OK, enough theory, what does this mean in practice? In particular, as a person whose day job involves working on PyTorch, I want to know what exactly should we be doing differently in this age of LLMs.  The path of least resistance is to just slot LLMs in anywhere in the traditional software development pipeline where they substitute only small steps out of a fundamentally human-oriented process.  But doing this quickly causes a bottleneck on TL reviewer bandwidth, where everyone is now able to generate code much more quickly, but LLMs haven't really improved the sense making / consensus-building / overall system design parts of the process (and arguably, the TLs had always been overloaded, and AI coding has only made it worse.)
