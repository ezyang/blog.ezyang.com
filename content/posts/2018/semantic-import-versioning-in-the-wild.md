---
title: "Semantic Import Versioning in the wild"
date: 2018-02-23 00:06:00
slug: semantic-import-versioning-in-the-wild
categories: [Programming Languages]
comments:
    - id: 22247
      author: "New top story on Hacker News: Semantic Import Versioning in the wild &#8211; Tech + Hckr News"
      date: "2018-02-23 05:47:41"
      content: "[&#8230;] Semantic Import Versioning in the wild 4 by panic | 0 comments on Hacker News. [&#8230;]"
    - id: 22248
      author: "New top story on Hacker News: Semantic Import Versioning in the wild &#8211; ÇlusterAssets Inc.,"
      date: "2018-02-23 06:08:36"
      content: "[&#8230;] Semantic Import Versioning in the wild 8 by panic | 0 comments on Hacker News. [&#8230;]"
    - id: 22249
      author: Maxim Ivanov
      date: "2018-02-23 08:12:46"
      content: "One of the most widely used examples of \"semantic import versioning\" you forgot to mention is shared libraries. /lib64/libz.so.1,  libncurses.so.5,  libc.so.6, ..."
    - id: 22250
      author: Edward Z. Yang
      date: "2018-02-23 10:46:40"
      content: |
        Hi Maxim, shared library versioning is a subtle one, which holds some similarities but not others (making it a bad analogy for semantic import versioning, but an interesting existing system to compare against.) The similarities: a compiled binary will be dynamically linked against a particular SOVERSION of a shared library (just like a semantic import), SOVERSIONs of a shared library are expected to always be ABI compatible (no BC-breaking changes.)
    ...
    - id: 22252
      author: Edward Z. Yang
      date: "2018-02-23 12:43:18"
      content: |
        Yeah, I pushed publish on this post before Russ's latest post :)
    ...
    - id: 22269
      ...
      date: "2018-02-26 19:01:15"
      content: "This also avoids up a whole collection of NP-complete problems Russ described in https://research.swtch.com/version-sat, which actually date back to Multics (plus Solaris, plus glibc) https://leaflessca.wordpress.com/2017/02/12/dll-hell-and-avoiding-an-np-complete-problem/"
    - id: 22296
      author: Justin Bailey
      date: "2018-03-16 12:59:35"
      content: |
...
- Semantic import versioning is widely practiced in programming languages, in the form of language standards/epochs. In C++, the setting of <span class="title-ref">-std=c++xx</span> specifies a particular semantic version to be "imported". It would be unheard of for a compiler to unilaterally break backwards compatibility of <span class="title-ref">-std=c++11</span> in a new revision of the compiler; similarly, a user must explicitly migrate to a new language standard to take advantage of any new features. [Rust epochs](https://github.com/rust-lang/rfcs/blob/master/text/2052-epochs.md) have a similar tenor. The choice between Python 2 and Python 3 is another form of semantic import versioning.
- Semantic imports don't have to just specify a number. Feature flags, such as <span class="title-ref">{-# LANGUAGE \#-}</span> pragmas in GHC Haskell, let users opt into BC-breaking changes at their use-sites.
...
3.  **Package level modularity.** In a traditional package management system, I can't release a single bundle of source code which presents multiple "package interfaces". Even in vgo, even if I have a shared codebase implementing <span class="title-ref">v1</span> and <span class="title-ref">v2</span>, I still have to make two releases to publish a new version of code. This is backwards; there is no reason a single unit of code cannot provide multiple interfaces, and package tools should make this possible.
